function checkXML(xml_value, attr_value, entities) {
	...
	if (window.DOMParser) {
		...
		if (null !== el.firstChild) {
			...
			if (null === el.getAttribute('attribute')) {
				return 0; //WebKit, attr
			} 
		}
		...
		//skip for potential IE error (validate entities in DTD)
		parser.validateOnParse = false;
		
		if (false === parser.loadXML(doc)) {
			return 0;
		}
	} else {
		return -1;
